...
export default function SideBar() {
  ...
  return (
    <div
      className={`absolute ${sidebarPosition === "left" ? "left-8" : "right-8"} top-1/2 -translate-y-1/2 z-50 flex flex-col items-center gap-2 ${toolbarPadding} ${sidebarColor} rounded-lg shadow-2xl border text-sm font-medium transition-colors sidebar-container ${
    ...
    >
      {isEnabled && (
        <div className="w-full text-center pb-2 border-b border-gray-500/30 mb-1 max-w-[80px] overflow-hidden">
          <span
            className="text-xs font-semibold truncate block px-1"
            title={
              typeof activePost?.content === "object" &&
              activePost.content !== null
                ? (activePost.content as unknown as Type).name
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={`flex items-center justify-center p-2 rounded-md transition-colors ${getButtonStyles("images")}`}
            >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={`flex items-center justify-center p-2 rounded-md transition-colors ${getButtonStyles("text")}`}
            >
